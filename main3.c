...
#define Q15_MAX 32767 // Fixed-point 16-bit number representation of fractions to replace floats
#define SAMPLE_RATE 8000 // Audio FIFO sample rate in Hz
#define PHASE_SCALE 536870.912  // 2^32/8000 to represent each step of phase, with 2^32 representing 2pi
...
// Divisor used to scale the rates above. Ranging from 0 - 10
int32_t attack_val = 10;
int32_t decay_val   = 5;
int32_t sustain_val = Q15_MAX;
int32_t release_val = 0;
...
// Structure representation of waves produced by each note pressed
typedef struct wave_struct {
    uint32_t phase;    // 0 to 2^32 representing 0 to 2pi
    int32_t output;    // Output is a fraction ratio by Q15
    uint32_t omega;    // omega = frequency * (2^32/SAMPLE_RATE)）
    bool is_playing;
    char adsr_stat;    // 'a'（attack），'d'（decay），'s'（sustain），'r'（release），'n'（none）
    int32_t adsr_multi; // Fraction ratio by Q15
} wave_struct;

// Use sine table for faster execution
...
// All playable notes on the keyboard
wave_struct waves[20] = {
    ...
    {0, 0, 250000000, false, 'n', 0}, // A#4, 466 Hz 
    {0, 0, 265000000, false, 'n', 0}, // B4, 494 Hz
    {0, 0, 280000000, false, 'n', 0}, // C5, 523 Hz
    {0, 0, 298000000, false, 'n', 0}, // C#5, 554 Hz
    {0, 0, 315000000, false, 'n', 0}, // D5, 587 Hz
    {0, 0, 334000000, false, 'n', 0}, // D#5, 622 Hz
    {0, 0, 354000000, false, 'n', 0}, // E5, 659 Hz
    {0, 0, 375000000, false, 'n', 0}, // F5, 698 Hz
    {0, 0, 398000000, false, 'n', 0}, // F#5, 740 Hz
    {0, 0, 421000000, false, 'n', 0}  // G5, 784 Hz
};
...
// Utilizing Q15 ratio to update the value of each wave at a new phase
...
short int Buffer1[240][512]; // VGA buffer
...
void ps2_wait_ready() {
    ...
    while (ps2_ptr->DATA & 0x8000); // bit15=1 means data busy
}
...
void ps2_init_keyboard() {
    ...
    // Wait for response of code 0xAA
    while (1) {
        if ((ps2_ptr->DATA & 0x8000) != 0) {
            uint8_t code = ps2_ptr->DATA & 0xFF;
            if (code == 0xAA) {
                led_ptr->LEDR = 0x8;  // Received code 0xAA
                delay(100000);
                break;
            }
        }
    ...
    led_ptr->LEDR = 0x4;  // Display success of sending 0xF4
    delay(100000);
}
...
// Used for displaying the wave
int wave_data_plot_y[140] = {0};


void update_wave_data_y() {
    // Use 32 bits to represent 0 to 2pi similar to previous
